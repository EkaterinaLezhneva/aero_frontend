...
import { GET_FILTERED_PRODUCTS_IN_SUCCESS, GET_FILTERED_PRODUCTS_IN_FAIL } from '../actions/productsFilterActionsTypes';
...
const productsReducer = createReducer(defaultState)
  .handleType(GET_FILTERED_PRODUCTS_IN_SUCCESS,
    (state: ProductsState, action: IInSuccessAction) => ({
      products: action.payload, error: null,
    }))
  .handleType(GET_FILTERED_PRODUCTS_IN_FAIL,
